...
class UserState:
    def __init__(self):
        ...
        self.application_type = "dual"  # "dual", "amg_only", "merso_only"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Hoş geldin mesajı - Başvuru türü seçimi
    welcome_text = """
🎯 <b>+18 Özel Kanala Hoş Geldiniz!</b>

━━━━━━━━━━━━━━━━━━━━━

📋 <b>Başvuru Türü Seçin:</b>

🔵 <b>Tam Başvuru:</b> Merso Bahis + AMG Bahis (Her ikisi de gerekli)
🟢 <b>Sadece AMG:</b> Sadece AMG Bahis başvurusu
🟡 <b>Sadece Merso:</b> Sadece Merso Bahis başvurusu

🔐 <b>Doğrulama Süreci:</b>
1️⃣ Seçtiğiniz site(ler) için bilgileri gönderin
2️⃣ Ekran görüntüsü + kullanıcı adı
3️⃣ Yönetici onayını bekleyin (5-15 dakika)

🚨 <u><b>ÇOK ÖNEMLİ:</b></u> 🚨
<u><b>MUTLAKA BİZİM LİNKİMİZLE</b></u> kayıt olun!
Başka linkle kayıt olursanız <b>başvurunuz reddedilir!</b>

━━━━━━━━━━━━━━━━━━━━━

<b>Hangi tür başvuru yapmak istiyorsunuz?</b> 👇
    """
    
    keyboard = [
        [InlineKeyboardButton("🔵 Tam Başvuru (Merso + AMG)", callback_data="app_type_dual")],
        [InlineKeyboardButton("🟢 Sadece AMG Başvuru", callback_data="app_type_amg_only")],
        [InlineKeyboardButton("🟡 Sadece Merso Başvuru", callback_data="app_type_merso_only")],
    ...
    if os.path.exists(photo_dir):
        ...
        if photo_files:
            ...
            # Her fotoğraf için farklı caption - Erotik & Genel Tema
            captions = [
                "🔥 <b>Sıcak İçerikler...</b>\n\nBunlar sadece başlangıç! Başvurunu tamamla ve özel +18 dünyasına katıl! 🔞",
                "💋 <b>VIP Deneyim Seni Bekliyor!</b>\n\nÖzel içerikler, sınırsız erişim... Başvurunu hemen yap! 💎",
                "😈 <b>Premium Erotik İçerikler...</b>\n\nHazırsan, özel topluluğumuza katıl ve keyfini çıkar! 🔥"
            ]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Başvuru türü seçimi
    if query.data.startswith("app_type_"):
        app_type = query.data.replace("app_type_", "")
        
        if user_id not in user_states:
            user_states[user_id] = UserState()
        
        # Başvuru türünü kaydet
        user_states[user_id].application_type = app_type
        
        if app_type == "dual":
            # Tam başvuru - önce Merso
            user_states[user_id].step = 1  # Merso screenshot
            site_url = SITES["merso"]["url"]
            site_name = SITES["merso"]["name"]
            site_key = "merso"
        elif app_type == "amg_only":
            # Sadece AMG
            user_states[user_id].step = 3  # AMG screenshot
            site_url = SITES["amg"]["url"]
            site_name = SITES["amg"]["name"]
            site_key = "amg"
        elif app_type == "merso_only":
            # Sadece Merso
            user_states[user_id].step = 1  # Merso screenshot
            site_url = SITES["merso"]["url"]
            site_name = SITES["merso"]["name"]
        ...
        # İlk site için başlat
        keyboard = [
            [InlineKeyboardButton(f"🌐 {site_name} Sitesine Git", url=site_url)],
            [InlineKeyboardButton("❌ İptal", callback_data="cancel")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        if app_type == "dual":
            message = f"<b>📍 AŞAMA 1/2: MERSO BAHIS</b>\n\n"
            message += f"<b>📝 Adım 1/2:</b> Siteye Üye Olun\n\n"
        else:
            message = f"<b>📍 {site_name.upper()} BAŞVURUSU</b>\n\n"
            message += f"<b>📝 Adım 1:</b> Siteye Üye Olun\n\n"
        
        message += f"🚨 <b><u>ÇOK ÖNEMLİ:</u></b> 🚨\n"
        message += f"Yukarıdaki butona tıklayarak <b><u>MUTLAKA BİZİM LİNKİMİZLE</u></b> kayıt olun!\n"
        message += f"Başka linkle kayıt olursanız <b>başvurunuz reddedilir!</b>\n\n"
        message += f"✅ <b>{site_name}</b> sitesine gidin\n"
        message += f"✅ Üye olun (henüz üye değilseniz)\n"
        message += f"✅ Hesabınıza giriş yapın\n\n"
        message += f"<b>📸 Adım 2:</b> Ekran Görüntüsü Gönderin\n\n"
        message += f"Üye olduktan ve giriş yaptıktan sonra:\n"
        message += f"• Ana sayfanızın ekran görüntüsünü alın\n"
        message += f"• Kullanıcı adınız görünür olmalı\n"
        message += f"• Ekran görüntüsünü buraya gönderin\n\n"
        message += f"⬇️ <b>{site_name} ekran görüntünüzü buraya gönderin:</b>"
    ...
    if query.data == "start_application":
        ...
        user_states[user_id].step = 1  # Merso screenshot
        user_states[user_id].application_type = "dual"  # Varsayılan tam başvuru
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text=f"<b>📍 AŞAMA 1/2: MERSO BAHIS</b>\n\n"
...
async def handle_application_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_state.step == 2:
        ...
        app_type = getattr(user_state, 'application_type', 'dual')
        
        if app_type == "merso_only":
            # Sadece Merso - başvuru tamamlandı
            with db_lock:
            ...
            if application_id:
                logger.info(f"Yeni Merso başvuru: ID={application_id}, User={username}, Merso={user_state.merso_username}")
                
                # Adminlere bildirim gönder
                admin_notification = (
                    f"🔔 <b>YENİ MERSO BAŞVURU!</b>\n\n"
                ...
                await update.message.reply_text(
                    "✅ <b>Merso Başvurunuz Tamamlandı!</b>\n\n"
                    f"🆔 Başvuru ID: <code>{application_id}</code>\n\n"
                    f"📋 <b>Bilgileriniz:</b>\n"
                    f"🏢 Merso: {user_state.merso_username}\n\n"
                    f"⏰ <b>Onay süresi:</b> 5-15 dakika\n"
                    f"📱 Sonuç size bildirilecektir.\n\n"
                    f"🔞 <b>Onay sonrası +18 kanala katılın!</b>",
                    reply_markup=reply_markup,
                    parse_mode=ParseMode.HTML
                )
        ...
        # AMG kullanıcı adı alındı - başvuru türüne göre tamamla
        user_state.amg_username = update.message.text.strip()
        
        # Başvuru türüne göre kaydet
        app_type = getattr(user_state, 'application_type', 'dual')
        
        if app_type == "dual":
            # Tam başvuru - her iki site
            with db_lock:
                application_id = db.add_application(
                    user_id=user_id,
                    username=username,
            ...
            if application_id:
                logger.info(f"Yeni AMG başvuru: ID={application_id}, User={username}, AMG={user_state.amg_username}")
                
                # Adminlere bildirim gönder
                admin_notification = (
                    f"🔔 <b>YENİ AMG BAŞVURU!</b>\n\n"
                    f"🆔 Başvuru ID: <code>{application_id}</code>\n"
                    f"🤖 Bot Kaynağı: <b>{BOT_SOURCE.upper()}</b>\n"
                    f"👤 Kullanıcı: {username}\n"
                    f"🆔 User ID: <code>{user_id}</code>\n\n"
            ...
            # Sadece Merso (bu durumda step 2'de tamamlanmış olmalı)
            pass
    ...
    if chat_member.new_chat_member.status in ['administrator', 'member']:
        ...
        if chat.type in ['group', 'supergroup']:
            ...
            if user.id != ADMIN_USER_ID:
                ...
                logger.warning(f"Yetkisiz kullanıcı tarafından gruba eklenmeye çalışıldı: {user.username} ({user.id}) - Grup: {group_title} ({group_id})")
                return
            
            # Grubu otomatik kaydet
            try:
                ...
                logger.info(f"Grup kaydedildi: {group_title} ({group_id})")
                
                # Bot sahibine ÖZEL MESAJ gönder
                notification_text = f"""
✅ <b>BOT2 KURULDU!</b>

Grup: <b>{group_title}</b>
...
/onayla [id] - Onayla
/reddet [id] - Reddet
/kirvebaslat - Tüm gruplara duyuru gönder
...
async def send_message_hybrid(update: Update, context: ContextTypes.DEFAULT_TYPE, welcome_message: str):
    """Hibrit mesaj gönderme: Grupta ise o gruba, özelden ise tüm gruplara"""
    ...
    # GRUPTA KULLANILDI: Sadece bu gruba mesaj gönder
    if chat_type in ['group', 'supergroup']:
        try:
            ...
            logger.info(f"Hoş geldin mesajı gönderildi (tek grup): {update.effective_chat.title} ({chat_id})")
    ...
    # ÖZELDEN KULLANILDI: Tüm kayıtlı gruplara mesaj gönder
    if chat_type == 'private':
        try:
            # Database'den tüm grupları al
            conn = db.sqlite3.connect(db.DATABASE_PATH)
            cursor = conn.cursor()
            cursor.execute("SELECT group_id, group_name FROM main_groups")
            groups = cursor.fetchall()
            conn.close()
            
            if not groups:
                await update.message.reply_text(
                    "⚠️ <b>Hiç kayıtlı grup yok!</b>\n\n"
                    "Botu bir gruba ekle ve admin yap.",
                    parse_mode=ParseMode.HTML
                )
                return
            ...
            success_count = 0
            fail_count = 0
            invalid_chats = []  # Geçersiz grupları topla
            
            for group_chat_id, chat_title in groups:
                try:
                    await context.bot.send_message(
                    ...
                    success_count += 1
                    logger.info(f"Hoş geldin mesajı gönderildi: {chat_title} ({group_chat_id})")
                except Exception as e:
                    fail_count += 1
                    error_msg = str(e)
                    
                    # "Chat not found" veya bot kicked out hatası
                    if "chat not found" in error_msg.lower() or "bot was kicked" in error_msg.lower():
                        invalid_chats.append(group_chat_id)
                        logger.warning(f"Geçersiz grup tespit edildi, silinecek: {chat_title} ({group_chat_id})")
                    else:
                        logger.error(f"Mesaj gönderme hatası ({chat_title}): {e}")
            
            # Geçersiz grupları database'den sil
            if invalid_chats:
                try:
                    conn_clean = db.sqlite3.connect(db.DATABASE_PATH)
                    cursor_clean = conn_clean.cursor()
                    for invalid_chat_id in invalid_chats:
                        cursor_clean.execute("DELETE FROM main_groups WHERE group_id = ?", (invalid_chat_id,))
                    conn_clean.commit()
                    conn_clean.close()
                    logger.info(f"{len(invalid_chats)} geçersiz grup database'den silindi")
                except Exception as e:
                    logger.error(f"Geçersiz grupları silerken hata: {e}")
            
            # Sonuç bildirimi
            result_msg = f"✅ <b>Mesaj Gönderildi!</b>\n\n"
            result_msg += f"📊 Başarılı: {success_count}\n"
            result_msg += f"❌ Başarısız: {fail_count}\n"
            result_msg += f"📁 Toplam: {len(groups)}"
            
            if invalid_chats:
                result_msg += f"\n\n🗑️ {len(invalid_chats)} geçersiz grup temizlendi"
            
            await update.message.reply_text(result_msg, parse_mode=ParseMode.HTML)
            
        except Exception as e:
            logger.error(f"Genel mesaj gönderme hatası: {e}")
            await update.message.reply_text(f"❌ Hata: {e}")

# ========== 4 FARKLI TEMA FONKSIYONU ==========

async def kitsune1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/kitsune1 - TEMA 1: Sıcak Karşılama"""
    # Sadece adminler kullanabilir
    if not await is_user_admin(update.effective_user.id, context):
        await update.message.reply_text("⚠️ Bu komutu sadece adminler kullanabilir.")
        return
    ...
    welcome_message = """
🔥 <b>Özel +18 Topluluğa Hoş Geldin!</b>

Sadece seçilmişler için... Yetişkin içerikli özel topluluğumuza katılmak ister misin?

<b>Giriş Şartları:</b>
• Merso Bahis aktif üyelik
• AMG Bahis aktif üyelik

<b>3 Basit Adım:</b>
1️⃣ Doğrulama butonuna tıkla
2️⃣ Üyelik belgelerini paylaş
3️⃣ 5-15 dakikada özel erişim al

<b>Hazırsan, başlayalım... 🔞</b>
    """
    
    await send_message_hybrid(update, context, welcome_message)

async def kitsune2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/kitsune2 - TEMA 2: VIP Lüks"""
    # Sadece adminler kullanabilir
    if not await is_user_admin(update.effective_user.id, context):
        await update.message.reply_text("⚠️ Bu komutu sadece adminler kullanabilir.")
        return
    ...
    welcome_message = """
👑 <b>VIP YETIŞKIN TOPLULUĞUNA HOŞ GELDIN!</b>

💋 <b>Seçkin Davet:</b>
Lüks +18 içeriklere özel erişim... Sadece VIP üyeler için!

💎 <b>VIP Giriş Gereksinimleri:</b>
• Merso Bahis VIP hesap
• AMG Bahis VIP hesap

⚡ <b>Hızlı VIP Erişim:</b>
1️⃣ VIP doğrulama başlat
2️⃣ Hesap belgelerini paylaş  
3️⃣ 5-15 dakikada özel içeriklere eriş

🔥 <b>Lüks dünyana adım at...</b>
    """
    
    await send_message_hybrid(update, context, welcome_message)

async def kitsune3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/kitsune3 - TEMA 3: Premium Çekici"""
    # Sadece adminler kullanabilir
    if not await is_user_admin(update.effective_user.id, context):
        await update.message.reply_text("⚠️ Bu komutu sadece adminler kullanabilir.")
        return
    ...
    welcome_message = """
💫 <b>PREMIUM EROTIK DÜNYAYA HOŞ GELDIN!</b>

🌹 <b>Elite Erişim:</b>
En sıcak +18 içerikler... Sadece premium üyeler için özel deneyim!

✨ <b>Premium Giriş:</b>
• Merso Bahis Premium hesap
• AMG Bahis Premium hesap

🎯 <b>Premium'a 3 Adım:</b>
① Premium doğrulamayı başlat
② Hesap bilgilerini onayla
③ Anında sınırsız erişim kazan

💋 <b>Tutkulu dünyaya katıl!</b>
    """
    
    await send_message_hybrid(update, context, welcome_message)

async def kitsune4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/kitsune4 - TEMA 4: Gizli Davet"""
    # Sadece adminler kullanabilir
    if not await is_user_admin(update.effective_user.id, context):
        await update.message.reply_text("⚠️ Bu komutu sadece adminler kullanabilir.")
        return
    ...
    welcome_message = """
🎭 <b>GİZLİ +18 TOPLULUĞA ÖZEL DAVET</b>

🔐 <b>Sadece Davetiyeyle:</b>
Sınırlı sayıda üyeye özel... Gizli yetişkin içerik topluluğuna davetlisin!

🎫 <b>Davetiye Gereksinimleri:</b>
• Merso Bahis aktif üyelik
• AMG Bahis aktif üyelik

🚪 <b>Gizli Kapıdan Giriş:</b>
▸ Özel davet kodunu al
▸ Kimlik doğrulamasını tamamla
▸ Gizli içeriklere anında eriş
...
def main():
    ...
    application.add_handler(CommandHandler("kitsune1", kitsune1))
    application.add_handler(CommandHandler("kitsune2", kitsune2))
    application.add_handler(CommandHandler("kitsune3", kitsune3))
    application.add_handler(CommandHandler("kitsune4", kitsune4))
    ...
    if True:
        async def send_startup_message():
            try:
                startup_message = (
                    f"🤖 <b>KirveBot 2 Başlatıldı!</b>\n\n"
                    f"📊 <b>Bot Bilgileri:</b>\n"
                    f"• Bot: Bot2 (Alternatif)\n"
                    f"• Sürüm: {BOT_VERSION}\n"
                    f"• Son Güncelleme: {LAST_UPDATE}\n"
                    f"• Durum: ✅ Aktif\n\n"
                    f"🔧 <b>Özellikler:</b>\n"
                    f"• Esnek başvuru türleri\n"
                    f"• AMG/Merso ayrı başvuru\n"
                    f"• Paylaşımlı database\n"
                    f"• Alternatif linkler\n\n"
                    f"💫 <b>Bot hazır ve çalışıyor!</b>"
                )
