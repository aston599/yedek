-- 🔧 CUSTOM COMMANDS TABLOSU DÜZELTME
-- Komut silinme sorununu çöz

-- 1. Eksik kolonları ekle
ALTER TABLE custom_commands ADD COLUMN IF NOT EXISTS response_message TEXT;
ALTER TABLE custom_commands ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;

-- 2. reply_text kolonunu response_message'a taşı (eğer varsa)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'custom_commands' AND column_name = 'reply_text'
    ) THEN
        UPDATE custom_commands 
        SET response_message = reply_text 
        WHERE response_message IS NULL AND reply_text IS NOT NULL;
        
        ALTER TABLE custom_commands DROP COLUMN IF EXISTS reply_text;
    END IF;
END $$;

-- 3. Unique constraint ekle
ALTER TABLE custom_commands 
DROP CONSTRAINT IF EXISTS custom_commands_unique;

ALTER TABLE custom_commands 
ADD CONSTRAINT custom_commands_unique 
UNIQUE(command_name, scope);

-- 4. Tüm komutları aktif yap
UPDATE custom_commands SET is_active = TRUE WHERE is_active IS NULL;

-- 5. Varsayılan komutları ekle (eğer yoksa)
...
-- 6. Index ekle
CREATE INDEX IF NOT EXISTS idx_custom_commands_name_scope ON custom_commands(command_name, scope);
CREATE INDEX IF NOT EXISTS idx_custom_commands_active ON custom_commands(is_active);

-- 7. Migration tamamlandı log'u
INSERT INTO system_logs (log_level, message, details) 
VALUES ('INFO', 'Custom commands table fixed', 'All columns and constraints updated successfully'); 